...
-- auth.users + auth.identities are both required for signInWithPassword to work.
do $$
declare
  pw text := 'SeedPass123!';
...
begin
  ...
  loop
    ...
    -- Replace any broken/partial identity from an earlier seed version.
...
-- Edge Functions (deploy separately; see supabase/deploy-edge-functions.sh):
--   overdue-tasks — POST { project_id } → overdue tasks with assignee names (RLS via caller JWT)
